...
$sqlt="SELECT * FROM feedback where Innovation_id='$innovation' and status='$status'";
  ...
  $reasonscoments_feedback=base64_decode(decrypt($row['reasons_feedback'], $key));
  $reasonstarget_feedback=base64_decode(decrypt($row['target_feedback'], $key));
  //$date_added=decrypt($row['date_added'], $key);
...
$sqreply="SELECT * FROM feedback where Innovation_id='$innovation' and status='$questions'";
 ...
 $target_feedback_reply=base64_decode(decrypt($row['target_feedback'], $key));
  //$date_added=decrypt($row['date_added'], $key);

}
 $show_butons="not_shown";
$commments=base64_encode(encrypt("my_replies",$key));
$checkExist=mysql_query("SELECT * FROM feedback where Innovation_id='$innovation' and status='$commments'") or die(mysql_error());
...
$sqlinforrr="SELECT * FROM innovation_expectation where Innovation_id='$innovation'";
    ...
    $query_runinforrr=mysql_query($sqlinforrr) or die($query_runinforrr."<br/><br/>".mysql_error());

    while($row=mysql_fetch_array($query_runinforrr)){
...
if($petnership_implementors && $patnership_innovators && $funding && $communities){
  ...
  $new_implementors=base64_decode(decrypt($petnership_implementors,$key));
  $new_innovators=base64_decode(decrypt($patnership_innovators,$key));
  $new_funding=base64_decode(decrypt($funding,$key));
  $new_communities=base64_decode(decrypt($communities,$key));
...
$checkExist=mysql_query("SELECT * FROM innovation_expectation WHERE Innovation_id='$innovation'") or die(mysql_error());
...
$sqlxo="SELECT * FROM innovation_expectation where Innovation_id='$innovation'";
    ...
    while($row=mysql_fetch_array($query_runxd)){
      ...
      if($petnership_implementors){
        ...
        $counting=$counting+1;
        $datadisplay=base64_decode(decrypt($petnership_implementors,$key));
        ?>
...
<?php
      ...
      if($patnership_innovators){
        ...
        $counting=$counting+1;
        $datadisplay=base64_decode(decrypt($patnership_innovators,$key));
        ?>
<div class="col-sm-12 col-xs-12 table_rowers">
  <div class="col-sm-1 col-sm-1"><?php echo $counting?></div>
  <div class="col-sm-11 col-sm-11"><?php echo $datadisplay?></div>
...
<?php
      ...
      $datadisplay="";
       ...
       if($funding){
        ...
        $counting=$counting+1;
        $datadisplay=base64_decode(decrypt($funding,$key));
        ?>
<div class="col-sm-12 col-xs-12 table_rowers">
  <div class="col-sm-1 col-sm-1"><?php echo $counting?></div>
  <div class="col-sm-11 col-sm-11"><?php echo $datadisplay?></div>
...
<?php
      ...
      $datadisplay="";
       ...
       if($communities){
        ...
        $counting=$counting+1;
        $datadisplay=decrypt($communities,$key);
        ?>
<div class="col-sm-12 col-xs-12 table_rowers">
  <div class="col-sm-1 col-sm-1"><?php echo $counting?></div>
  <div class="col-sm-11 col-sm-11"><?php echo $datadisplay?></div>
...
$.post("modules/system/external/pages/feedback/save.php",{
    ...
    if($.trim(data)=="success"){
        ...
        $.post("modules/system/external/pages/feedback/feedback_view.php",{
            my_id:my_id,
            innovation:innovation
        },function(data){
        $("#datasendserrordddddd").html("");
...
var my_id=$("#global_u_email").val();
  //alert(innovation);
var url="modules/system/external/pages/feedback/feedback.php";
$.post(url,{my_id:my_id},function(data){$("#home").html(data)});
})
    })
  </script>