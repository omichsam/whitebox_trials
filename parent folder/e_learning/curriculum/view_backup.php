...
$counter=0;
$sql="SELECT * FROM curriculum_course_outline Where unit_id='$role'";
    $query_run=mysqli_query($con,$sql) or die($query_run."<br/><br/>".mysqli_error($con));

    while($row=mysqli_fetch_array($query_run)){
 $course_outline_name =$row['course_outline_name'];
  $courseoutline_id =$row['id'];
  $counter=$counter+1;
    	?>
    	<div class=" col-sm-12 col-xs-12 open_course" id="<?php echo $courseoutline_id?>"><?php echo "$role.$counter: $course_outline_name"?></div>
<?php
}
?>
<div class="col-sm-12 col-xs-12" style="text-align: left;font-weight: bold;font-size: 20px;margin-top: 20px;">Other Units</div>
<?php
...
<script type="text/javascript">
	$(document).ready(function(){
		$(".open_course").click(function(){
			...
			$.post("e_learning/curriculum/load.php",{role:role},function(data){
