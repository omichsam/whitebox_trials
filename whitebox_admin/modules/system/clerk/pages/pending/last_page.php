...
$query_runxdol=mysqli_query($con,$sqlxol) or die($query_runxdol."<br/><br/>".mysqli_error($con));
  ...
  $reasons_feedback_question=$row['reasons_feedback'];

$query_resaoce=$reasons_feedback_question;
}
if($reasons_feedback_question){
$check_reasonce="Question";
...
$sqlxo="SELECT * FROM feedback where Innovation_id='$innovation' and status='$commments'";
  ...
  $reasondds_feedback=$row['reasons_feedback'];
$Clerk_id=$row['Clerk_id'];
if($reasondds_feedback){
$newreasonsds_feedback=$reasondds_feedback;
}else{

}

}
$sqlxo="SELECT reasons_feedback,Clerk_id FROM feedback where Innovation_id='$innovation' and status='$evaluate_replys'";
...
$sqlxol="SELECT reasons_feedback FROM feedback where Innovation_id='$innovation' and status='$clerks_question'";

$query_runxdol=mysqli_query($con,$sqlxol) or die($query_runxdol."<br/><br/>".mysqli_error($con));

    while($row=mysqli_fetch_array($query_runxdol)){
  $reasoncesd_question=$row['reasons_feedback'];





if($reasoncesd_question){

$query_question="Yes";
$show_question="";
$newreasoncesd_question=$reasoncesd_question;
}else{
...
<div class="col-sm-12 col-xs-12 no_padding">
  ...
  <?php echo $query_resaoce?>
</div>
 <div class="col-sm-12 col-xs-12" id="innovationneed">
<?php echo $newreasonsds_feedback?>
</div>
</div>
...
$(".my_explainquestions").hide()
    ...
    var urla="modules/system/clerk/pages/pending/decide.php";
...
if($.trim(data)=="exist"){
    $.post(""+url+"",{
      ...
      $("#targeted_dcmodel").html("Say something here").css("color","red");
}
...
//END NO QUERY
  ...
  var check_reasonce='<?php echo $check_reasonce?>';
if(check_reasonce==""){
 var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);

    $.post("modules/system/clerk/pages/pending/decide.php",{innovation:innovation},function(data){
    $(".evaluation_class").hide();
    $("#decide").html(data).show();
    });
