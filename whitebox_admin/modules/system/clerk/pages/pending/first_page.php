...
$sqlxol="SELECT need_feedback,Clerk_id FROM feedback where Innovation_id='$innovation' and status='$questions'";
...
$sqlxo="SELECT need_feedback FROM feedback where Innovation_id='$innovation' and status='$status'";
...
$sqlxo="SELECT need_feedback FROM feedback where Innovation_id='$innovation' and status='$commments'";
    ...
    while($row=mysqli_fetch_array($query_runxd)){
  $need_feedbackcoments=$row['need_feedback'];
//$Clerk_id=$row['Clerk_id'];
...
if($need_feedbackcoments){
$newneedc_feedback=$need_feedbackcoments;
...
$sqlxol="SELECT need_feedback FROM feedback where Innovation_id='$innovation' and status='$clerks_question'";

$query_runxdol=mysqli_query($con,$sqlxol) or die($query_runxdol."<br/><br/>".mysqli_error($con));

    while($row=mysqli_fetch_array($query_runxdol)){
  $needd_question=$row['need_feedback'];

if($needd_question){
$query_question="Yes";
$show_question="";
$nasked_question=$needd_question;
}else{
...
<div class="col-sm-12 col-xs-12 no_padding">
  ...
  <?php echo $newneed_question ?>
</div>
 <div class="col-sm-12 col-xs-12" id="innovationneed">
<?php echo $newneedc_feedback ?>
</div>
</div>
<!--<div class="col-sm-12 col-xs-12">
<div class="col-sm-12 no_padding col-xs-12" id="need_clarify">
  ...
  <div class="no_padding my_explainquestions <?php echo $show_question?> col-sm-12 col-xs-12">
    ...
    <textarea class=" col-sm-12 col-xs-12 questions_hold" id="need_reply" maxlength="200" onkeyup="questionone(this.value)" placeholder="E.g Can i get to know more on the need?"><?php echo $nasked_question?></textarea>
...
$(".my_explainquestions").hide()
    ...
    var urla="modules/system/clerk/pages/pending/second_page.php";
...
//END NO QUERY
     ...
     var innovation='<?php echo $innovation?>';
    var my_url="modules/system/clerk/pages/pending/evaluate.php";
...
var check_need='<?php echo $check_need?>';
if(check_need==""){
 var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);

    $.post("modules/system/clerk/pages/pending/second_page.php",{innovation:innovation},function(data){
