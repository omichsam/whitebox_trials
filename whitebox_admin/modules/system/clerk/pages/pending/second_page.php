...
$sqlxol="SELECT * FROM feedback where Innovation_id='$innovation' and status='$questions'";
...
$query_runxdol=mysqli_query($con,$sqlxol) or die($query_runxdol."<br/><br/>".mysqli_error($con));
  ...
  $solution__question=$row['solution_feedback'];
$Clerk_id=$row['Clerk_id'];
}
if($solution__question){
$check_solustions="Question";
...
$solution_query="";
$sqlxo="SELECT solution_feedback FROM feedback where Innovation_id='$innovation' and status='$evaluate_replys'";
...
$sqlxo="SELECT solution_feedback FROM feedback where Innovation_id='$innovation' and status='$questions'";

    $query_runxd=mysqli_query($con,$sqlxo) or die($query_runxd."<br/><br/>".mysqli_error($con));

    while($row=mysqli_fetch_array($query_runxd)){
  $soquz_feedback=$row['solution_feedback'];

if($soquz_feedback){
$solution_query=$soquz_feedback;
}else{
...
$sqlxol="SELECT solution_feedback FROM feedback where Innovation_id='$innovation' and status='$clerks_question'";
...
<div class="col-sm-12 not_shown col-xs-12 process_tm">
...
$(".my_explainquestions").hide()
    ...
    var urla="modules/system/clerk/pages/pending/third_page.php";
...
if($.trim(data)=="exist"){
    $.post(""+url+"",{
      ...
      $("#solution_dc").html("Say something here").css("color","red");
}
...
var check_solustions='<?php echo $check_solustions?>';
if(check_solustions==""){
 var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);

    $.post("modules/system/clerk/pages/pending/third_page.php",{innovation:innovation},function(data){
    $(".evaluation_class").hide();
