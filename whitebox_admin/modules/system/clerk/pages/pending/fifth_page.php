...
$sqlxol="SELECT businessmodel_feedback FROM feedback where Innovation_id='$innovation' and status='$questions'";
...
$query_runxdol=mysqli_query($con,$sqlxol) or die($query_runxdol."<br/><br/>".mysqli_error($con));
  ...
  $businessmodel_feedback_question=$row['businessmodel_feedback'];

$query_models=$row['businessmodel_feedback'];
}
if($businessmodel_feedback_question){
$check_bmodel="Question";
...
$sqlxo="SELECT businessmodel_feedback,Clerk_id FROM feedback where Innovation_id='$innovation' and status='$evaluate_replys'";

    $query_runxd=mysqli_query($con,$sqlxo) or die($query_runxd."<br/><br/>".mysqli_error($con));

    while($row=mysqli_fetch_array($query_runxd)){
  $impact_feedbacmodelkcoments=$row['businessmodel_feedback'];
...
if($impact_feedbacmodelkcoments){
$newimpact_ddmodelfeedback=$impact_feedbacmodelkcoments;
}else{

}

}
$sqlxol="SELECT businessmodel_feedback FROM feedback where Innovation_id='$innovation' and status='$clerks_question'";
...
<textarea id="commentmodel_solution" maxlength="400" onkeyup="coment_tmodel(this.value)" placeholder="Type your coments about the subject here max(400 characters)" class="coments col-sm-12 col-xs-12"><?php echo $newimpact_ddmodelfeedback?></textarea>
...
$(".my_explainquestions").hide()
    ...
    var urla="modules/system/clerk/pages/pending/sixth_page.php";
...
var commentmodel_solution=$("#commentmodel_solution").val()

        if(commentmodel_solution){
        
    var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);
        var coments=btoa(commentmodel_solution)
...
if($.trim(data)=="exist"){
    $.post(""+url+"",{
      ...
      $("#targem_dcmodel").html("Say something here").css("color","red");
}
...
//END NO QUERY
    ...
    var check_bmodel='<?php echo $check_bmodel?>';
if(check_bmodel==""){
 var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);

    $.post("modules/system/clerk/pages/pending/sixth_page.php",{innovation:innovation},function(data){
    $(".evaluation_class").hide();
    $("#sixth_page").html(data).show();
    });
