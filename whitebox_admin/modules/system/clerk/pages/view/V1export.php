...
//get rows query
$sqlx = "SELECT user_id,sector_id,InnovationBig4Sector,Innovation_name,stage,need,solution,impact,busines_model,target,Research_sources,requirements FROM innovations_table Where Status='submission'";
...
$getbigid=mysqli_fetch_assoc($get_bigfoursectors);
 ...
 $bg4id_name=$getbigid['Name'];
    ...
    $post_list[] = array( "No"=>$sno, "Innovator name"=>$first_name." ".$last_name, "Email addres"=>$email, "Phone number"=>$phone, "Innovation_name"=>$row["Innovation_name"], "Sector"=>$newsector, "Big four Agenda"=>$bg4id_name, "stage"=>$row["stage"],"need"=>$row["need"],  "solution"=>$row["solution"],"impact"=>$row["impact"],  "busines_model"=>$row["busines_model"],"target"=>$row["target"],  "Research_sources"=>$row["Research_sources"],"requirements"=>$row["requirements"]);
    $sno++;
  }
}
