...
if($loginuser){
	if($loginpass){
		$pass=$loginpass;
 		$checkExist=mysqli_query($con,"SELECT user_name,pass_key FROM administrators WHERE user_name='$new_user' AND pass_key='$new_pass'") or die(mysqli_error($con));

  		if(mysqli_num_rows($checkExist)!=0){
  		$checkaxtiv=mysqli_query($con,"SELECT user_name,pass_key FROM administrators WHERE user_name='$new_user' AND pass_key='$new_pass' and status ='$statusof'") or die(mysqli_error($con));
  		...
  		$get_user=mysqli_query($con,"SELECT * FROM administrators WHERE user_name='$new_user' AND pass_key='$new_pass'") or die(mysqli_error($con));
