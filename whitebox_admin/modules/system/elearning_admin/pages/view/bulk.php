    ...
    if($FileType != "csv" || $FileType != "xlsx" || $FileType != "xltx" || $FileType != "xltm" || $FileType != "xlsm" || $FileType != "xls"){
        ...
        if (move_uploaded_file($_FILES["csv_file"]["tmp_name"], $target_file)) {
           ...
           $Account_type="clerk";
            ...
            $date = time();
         $new_time=$date;
         $checkExist=mysqli_query($con,"SELECT user_name FROM e_learning_admin WHERE user_name='$email'") or die(mysqli_error($con));
  ...
  if(mysqli_num_rows($checkExist)>=1){
           ...
           VALUES (null,
                   ...
                   '1')") or die(mysqli_error($con));
                    //echo "sucess";
           if($query){
          $message="<p>Dear $names</p><p>Your innovation portal acoount has been created. Kindly access the WhiteBox Innovation portal  via the link: https://www.whitebox.go.ke/whitebox_admin.</p><p>Your account cridentials are:</p><p> Username: $row[2] </p><p> password: $code</p><p>Regards</p><p>Huduma WhiteBox</p>";  
...
if($mail->Send()){
  ...
  echo "Sorry, kindly choose a file to upload.";
}
?>
