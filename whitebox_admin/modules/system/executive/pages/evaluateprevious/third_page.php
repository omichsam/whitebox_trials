...
$sqlxo="SELECT impact FROM   innovation_details where Innovation_Id='$innovation'";
$counter=0;
...
<div class="col-sm-12 col-xs-12 evaluate_header" id="">EVALUATION STAGE THREE</div>
<div class="col-sm-12 col-xs-12 no_padding">
...
<textarea id="comment_impact" onkeyup="impactdupdate(this.value)" class="coments col-sm-12 col-xs-12"></textarea>
<div class="colsm-12 col-xs-12" style="text-align: center; " id="impact_check"></div>
</div>
...
<div class="col-sm-4 col-xs-12 "><span class="btn evaluate_impact theme_bg" role="back">Back</span> <!--<span class="btn evaluate_impact theme_bg">Disapprove</span>--> <span class="btn evaluate_impact theme_bg" role="fourth_page">Next Step</span> </div>

</div>

<div class="col-sm-12 col-xs-12 " style="text-align: center; " id="third_error">&nbsp;</div>
...
<script type="text/javascript">
    function impactdupdate(str) {
  var lng = str.length;
  document.getElementById("impact_check").innerHTML = lng + ' out of 400 characters';
}
  $(document).ready(function(){
    var loader=$("#loader").html()
    var display="#third_error";
    var third_target="#fourth_page";
    var folder=".evaluation_class";
    var innovation=btoa('<?php echo $innovation?>');
    var urlthird_save="modules/system/clerk/pages/evaluate/save.php";
    var urlfourth="modules/system/clerk/pages/evaluate/fourth_page.php";
    $(".evaluate_impact").click(function(){
      var user=$("#user_email").val();
      var my_role=$(this).attr("role");
      if(my_role=="fourth_page"){
 ...
 var field=btoa("impact_feedback");
    ...
    var innovation=btoa(innovats);
        ...
        //alert()
      $(display).html(loader);
    $.post(""+urlthird_save+"",{field:field,innovation:innovation,coments:coments,user:user},function(data){ if($.trim(data)=="success"){$.post(""+urlfourth+"",{innovation:innovation},function(data){$(folder).hide();$(display).html("");$(third_target).show().html(data);})}else{$(display).html(data);}})
  }else{
    //alert()
 var field=btoa("impact_feedback");
...
if($.trim(data)=="exist"){
        ...
        $.post(""+urlfourth+"",{innovation:innovation},function(data){$(folder).hide();
        $(third_target).html(data).show();})

}else{
//alert()
      $("#impact_check").html("Say something here").css("color","red");
}
