...
$sqlxo="SELECT solution FROM   innovation_details where Innovation_Id='$innovation'";
$counter=0;
...
<div class="col-sm-12 col-xs-12 evaluate_header" id="">EVALUATION STAGE TWO</div>
<div class="col-sm-12 col-xs-12 no_padding">
...
<textarea id="comment_solution" onkeyup="solutiondupdate(this.value)" class="coments col-sm-12 col-xs-12"></textarea>
<div class="colsm-12 col-xs-12" style="text-align: center; " id="solution_check"></div>
</div>
...
<div class="col-sm-4 col-xs-12 "><span class="btn evaluate_need theme_bg" role="back">Back</span> <!--<span class="btn evaluate_need theme_bg">Disapprove</span>--> <span class="btn evaluate_need theme_bg" role="third_page">Next Step</span> </div>

</div>

<div class="col-sm-12 col-xs-12 " style="text-align: center; " id="second_error">&nbsp;</div>
...
<script type="text/javascript">
    function solutiondupdate(str) {
  var lng = str.length;
  document.getElementById("solution_check").innerHTML = lng + ' out of 400 characters';
}
  $(document).ready(function(){
    var loader=$("#loader").html()
    var display="#second_error";
    var second_target="#third_page";
    var folder=".evaluation_class";
    var innovation=btoa('<?php echo $innovation?>');
    var urlsecond_save="modules/system/clerk/pages/evaluate/save.php";
    var urlthird="modules/system/clerk/pages/evaluate/third_page.php";
    $(".evaluate_need").click(function(){
      var user=$("#user_email").val();
      var my_role=$(this).attr("role");
      if(my_role=="third_page"){
    ...
    var innovation=btoa(innovats);
        ...
        //alert()
      $(display).html(loader);
    $.post(""+urlsecond_save+"",{field:field,coments:coments,innovation:innovation,user:user},function(data){ if($.trim(data)=="success"){$.post(""+urlthird+"",{innovation:innovation},function(data){$(folder).hide();$(display).html("");$(second_target).show().html(data);})}else{$(display).html(data);}})
  }else{
 var field=btoa("solution_feedback");
...
if($.trim(data)=="exist"){
        ...
        $.post(""+urlthird+"",{innovation:innovation},function(data){$(folder).hide();
        $(second_target).html(data).show();})

}else{

      $("#solution_check").html("Say something here").css("color","red");
}
