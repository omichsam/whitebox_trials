...
$counter=0;
 ...
 $counter=$counter+1;
   $sqldp="SELECT * FROM external_users WHERE User_id='$user_id'";
    $query_rundp=mysql_query($sqldp)or die($query_rundp."<br/><br/>".mysql_error());
        while($row=mysql_fetch_array($query_rundp)){
          $First_name=base64_decode(decrypt($row['First_name'], $key));
          $Last_name=base64_decode(decrypt($row['Last_name'], $key));
