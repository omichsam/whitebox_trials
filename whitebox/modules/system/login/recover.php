...
$check_exists=mysqli_query($con,"SELECT user_name,Name FROM e_learning_admin WHERE user_name='$thisUser'") or die(mysqli_error($con));
if(mysqli_num_rows($check_exists)>=1){
    
    	$checkaxtiv=mysqli_query($con,"SELECT user_name,pass_key FROM e_learning_admin WHERE user_name='$thisUser' and status ='$statusof'") or die(mysqli_error($con));
