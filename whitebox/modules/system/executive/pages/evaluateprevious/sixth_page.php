...
$sqlxo="SELECT attachments FROM innovations_information where Innovation_Id='$innovation'";
$counter=0;
...
<div class="col-sm-12 col-xs-12 evaluate_header" id="">EVALUATION STAGE SIX</div>
<div class="col-sm-12 col-xs-12 no_padding">
<div class="col-sm-8 col-xs-12 ">
  ...
  <div class="col-sm-12 colxs-12 default_header">BUSINESS attachmentS</div>
...
<textarea id="comment_attachments" onkeyup="attachmentsdupdate(this.value)" class="coments col-sm-12 col-xs-12"></textarea>
<div class="colsm-12 col-xs-12" style="text-align: center; " id="attachments_check"></div>
</div>
...
<div class="col-sm-4 col-xs-12 "><span class="btn evaluate_attachment theme_bg" role="back">Back</span> <!--<span class="btn evaluate_attachment theme_bg">Disapprove</span>--> <span class="btn evaluate_attachment theme_bg" role="last_page">Finish</span> </div>

</div>

<div class="col-sm-12 col-xs-12 " style="text-align: center; " id="fifth_error">&nbsp;</div>
...
<script type="text/javascript">
    function attachmentsdupdate(str) {
  var lng = str.length;
  document.getElementById("attachments_check").innerHTML = lng + ' out of 400 characters';
}
  $(document).ready(function(){
    var loader=$("#loader").html()
    var display="#fifth_error";
    var sixth_target="#last_page";
    var folder=".evaluation_class";
    var innovation=btoa('<?php echo $innovation?>');
    var urlfifth_save="modules/system/clerk/pages/evaluate/save.php";
    var urlsix="modules/system/clerk/pages/evaluate/last_page.php";
    $(".evaluate_attachment").click(function(){
      var user=$("#user_email").val();
      var my_role=$(this).attr("role");
      if(my_role=="last_page"){
        var comment_attachments=$("#comment_attachments").val()
        if(comment_attachments){
          
 var field=btoa("attachments_feedback");
    var innovats='<?php echo $innovation?>';
    var innovation=btoa(innovats);
        var coments=btoa(comment_attachments);

        //alert()
      $(display).html(loader);
    $.post(""+urlfifth_save+"",{field:field,innovation:innovation,coments:coments,user:user},function(data){ if($.trim(data)=="success"){$.post(""+urlsix+"",{innovation:innovation},function(data){$(folder).hide();$(display).html("");$(sixth_target).show().html(data);})}else{$(display).html(data);}})
  }else{
    //alert()
 var field=btoa("attachments_feedback");
...
if($.trim(data)=="exist"){
        ...
        $.post(""+urlsix+"",{innovation:innovation},function(data){$(folder).hide();
        $(sixth_target).html(data).show();})

}else{
//alert()
      $("#attachments_check").html("Say something here").css("color","red");
}
