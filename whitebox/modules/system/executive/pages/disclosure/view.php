...
margin-bottom:10px;
    ...
    #headers_logo{	
		...
		background:url('http://www.tumcathcom.com/museum/images/logo.png');
		border-radius:20px;
    }
    .herihubbb{
        text-align:center;
...
$date=date('Y-m-d');
$my_userde=base64_decode($_POST['user']);
$get_user=mysqli_query($con,"SELECT * FROM investors WHERE Email='$my_userde'") or die(mysqli_error($con));
$get=mysqli_fetch_assoc($get_user);
$new_First_name=$get['Name'];
$new_company=$get['Company'];
$new_Location=$get['Location'];
$new_contact=$get['Contact'];
$new_interest=$get['interest'];
$prof_rolesd=$get['role'];
$email=$get['Email'];
$Approved_date=$get['Approved_date'];
$approved=date('m/d/Y H:i:s', $Approved_date);
$agrement_date=$get['agrement_date'];
$applied=date('m/d/Y H:i:s', $agrement_date);
//$approved=strtotime($afterdate);
$image=$get['image'];
if($image){
   $new_pic="http://www.tumcathcom.com/museum/".$image; 
}else{
    $new_pic="http://www.tumcathcom.com/museum/images/user.jpg";
}
...
<div class="col-sm-10 col-xs-10 no_padding herilocaled jutict" id="">
<div class="col-sm-6 col-xs-6 data_utp " > 
<div class="col-sm-12 col-xs-12 no_padding" style="font-weight:bolder">
Signed for and on behalf of HeriHub   
</div>
 <div class="col-sm-12 col-xs-12 no_padding data_utp">
  <div class="col-sm-2 col-xs-2 no_padding">Name<span style="float:right">:</span></div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt"><?php echo "Jacqueline Macharia"?></div>
 </div>
<div class="col-sm-12 col-xs-12 no_padding data_utp ">
  <div class="col-sm-2 col-xs-2 no_padding">Position:</div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt"><?php echo "Administrator"?></div>
 </div>
 <div class="col-sm-12 col-xs-12 no_padding data_utp">
  <div class="col-sm-2 col-xs-2 no_padding">Date<span style="float:right">:</span></div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt"><?php echo $approved?></div>
 </div>
</div> 
    
  <div class="col-sm-6 col-xs-6 data_utp " > 
<div class="col-sm-12 col-xs-12 no_padding" style="font-weight:bolder;text-transform:lowercase;">
Signed for <?php echo $new_company ?>  
</div>
 <div class="col-sm-12 col-xs-12 no_padding data_utp">
  <div class="col-sm-2 col-xs-2 no_padding">Name<span style="float:right">:</span></div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt" style="text-transform:sentence-cased"><?php echo $new_First_name?></div>
 </div>
<div class="col-sm-12 col-xs-12 no_padding data_utp ">
  <div class="col-sm-2 col-xs-2 no_padding">Position:</div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt"><?php echo $prof_rolesd ?></div>
 </div>
 <div class="col-sm-12 col-xs-12 no_padding data_utp">
  <div class="col-sm-2 col-xs-2 no_padding">Date<span style="float:right">:</span></div>   
  <div class="col-sm-10 col-xs-10 no_padiing content_cgt"><?php echo $applied?></div>
 </div>
</div>  
...
<span class="btn theme_bg disclo_btnsd" role="back">Back</span> </div>  
  <div class="col-sm-12 col-xs-12" id="disclosure_error" style="text-align:center"></div>  
    
</div>
</div>

<div class=" col-sm-12 col-xs-12" style="min-height:400px"></div>
<div class="topped col-sm-12 col-xs-12"></div>
    </div>
             ...
             //  alert(role)
               if(role=="back"){
                ...
                $("#disclosure_error").html(loader);
                  $.post("modules/system/executive/pages/disclosure/disclosure.php",function(data){
                $("#home").html(data);
            })  
               }else{
