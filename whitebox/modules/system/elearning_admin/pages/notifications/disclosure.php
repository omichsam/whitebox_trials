...
$email=base64_decode(decrypt($get['Email'],$key));
$image=$get['image'];
if($image){
   $new_pic="http://www.tumcathcom.com/museum/".base64_decode(decrypt($image,$key)); 
}else{
...
<span class="btn theme_bg disclo_btnsd" role="back">Back</span>  <span class="disclo_btnsd btn theme_bg" role="Declined">Decline</span> <span class="btn disclo_btnsd theme_bg" role="Approved">Approve</span>
</div>  
             ...
             //  alert(role)
               if(role=="back"){
                ...
                $("#disclosure_error").html(loader);
                  $.post("modules/system/elearning_admin/pages/notifications/notifications.php",function(data){
                $("#home").html(data);
            })  
               }else{
               var my_id='<?php echo $my_userde ?>';
               var myrole=btoa(role);
                  $.post("modules/system/elearning_admin/pages/notifications/update_role.php",{
                      my_id:my_id,
                      myrole:myrole
                      },function(data){
